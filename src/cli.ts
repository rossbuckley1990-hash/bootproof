...
async function main() {
  ...
  if (cmd === "up") {
    const opts: UpOptions = {
      provider: (flags.provider as "docker" | "local") ?? "docker",
    ...
    if (flags.json) return console.log(JSON.stringify({ inference: outcome.inference, plan: outcome.plan, refusal: outcome.refusal, attestation: outcome.attestation }, null, 2));
  ...
  if (cmd === "verify") {
    const p = attestationPath(target);
...
main().catch(err => { bad(String(err?.message ?? err)); process.exitCode = 1; });
